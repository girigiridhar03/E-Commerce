...
const SpeakerPage = () => {
  ...
  const filterProduct = selectedBox.length === 0 ? speakerData : speakerData.filter(item=>selectedBox.includes(item.brand));
...
<div className='h-[500px] w-[40%] md:w-[30%] border-2 border-red-500 rounded-lg mt-2 py-3 px-2'>
 ...
 <h4 className='text-yellow-500 text-sm md:text-lg mb-2 font-bold  border-b-2 inline-block border-red-500'>FILTER</h4>
  ...
  {speakerData.map(item=>{
    return <div key={item.model} className='mt-2'>
 ...
 {filterProduct.map(item=>{
     return <div key={item.model} className='flex border-2 border-yellow-500 overflow-hidden flex-col p-1 group rounded-lg lg:p-3 w-[100%] md:w-[30%]'>
      <Link to={`/speakers/${item.model}`}>
