...
const ComputerPage = () => {
  ...
  const filterProduct = selectedBox.length === 0 ? computerData : computerData.filter(item=>selectedBox.includes(item.company));
  ...
  {computerData.map(item=>{
    return <div key={item.image} className='mt-2'>
        <label className='text-yellow-500 flex items-center gap-5 font-bold text-sm md:text-lg'>
          <input type="checkbox" checked = {selectedBox.includes(item.company)} onChange={()=>handleOnChange(item.company)}/>
        {item.company}
        </label>
    </div>
  })}
