...
const Fruniture = () => {
    ...
    return (
      ...
      <div>
       <h1 className='text-lg font-bold text-yellow-500 lg:text-2xl'>Computer</h1>
           ...
           return <div key={item.id} className='flex border-2 border-yellow-500 overflow-hidden flex-col p-1 group rounded-lg lg:p-3'>
            ...
            <div className='w-[100%] overflow-hidden'>
             <img src={item.image} alt="" className='rounded-lg w-[100%]  group-hover:scale-95 transition ease-in-out cursor-pointer'/>
              ...
              <div className='text-[10px] md:px-2 py-2 font-bold sm:text-sm lg:text-lg inline-flex mt-auto'>
                   <p>{item.product}</p>
                   <p>{item.company}</p>
                   <p>RS.{item.price}</p>
              </div>
