...
const Speakers = () => {
    ...
    return (
      ...
      <div>
       <h1 className='text-lg font-bold text-yellow-500 lg:text-2xl'>Speaker's</h1>
       
       <div className='flex gap-3 px-4 py-2 text-red-500 xl:justify-between'>
       {firstFiveSpeakers.map(item=>{
           return <div key={item.image} className='flex border-2 border-yellow-500 overflow-hidden flex-col p-1 group rounded-lg lg:p-3'>
              <Link to={`/${item.model}`}>
